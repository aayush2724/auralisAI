...
def built_vectorstore(tmp_dirs, sample_md):
    ...
    _reset_cache()
    n_chunks, _ = ingest_directory(
        data_dir, vectorstore_path=vs_dir, audience="internal"
    )
...
class TestRetrieve:
    ...
    def test_audience_filtering(self, built_vectorstore):
        ...
        assert (
            len(results) == 0
        ), "Expected zero results because all chunks are internal and we filtered for external"
    ...
    def test_stitching_audience_filtering(self, built_vectorstore):
        ...
        doc1 = Document(
            page_content="External chunk zero",
            metadata={
                "source_file": "stitching_test.txt",
                "chunk_index": 0,
                "audience": "external",
            },
        )
        doc2 = Document(
            page_content="Internal chunk one",
            metadata={
                "source_file": "stitching_test.txt",
                "chunk_index": 1,
                "audience": "internal",
        ...
        doc3 = Document(
            page_content="External chunk two",
            metadata={
                "source_file": "stitching_test.txt",
                "chunk_index": 2,
                "audience": "external",
        ...
        target_res = next(
            (
                r
                for r in results
                if r["source_file"] == "stitching_test.txt" and r["chunk_index"] == 0
            ),
            None,
        )
        ...
        assert (
            "Internal chunk one" not in stitched_text
        ), "Internal neighbor was improperly stitched!"
