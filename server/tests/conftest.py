...
if os.getenv("USE_MOCK_LLM", "").lower() in ("1", "true", "yes"):
    ...
    def fake_classify(self, text: str, candidate_labels: list[str], **kwargs):
        ...
        if candidate_labels and (
            "competitor" in candidate_labels or "price" in candidate_labels
        ):
            if (
                "hubspot" in t
                or "hub spot" in t
        ...
        elif candidate_labels and (
            "CEO" in candidate_labels or "CTO" in candidate_labels
        ):
            ...
            "positive" in candidate_labels or "negative" in candidate_labels
        ):
            if any(
                w in t
                for w in (
    ...
    class FakeEmbeddings(Embeddings):
        ...
        def embed_documents(self, texts: list[str]) -> list[list[float]]:
            return [
                [float((i + 1) % 10) for _ in range(768)] for i, _ in enumerate(texts)
            ]
