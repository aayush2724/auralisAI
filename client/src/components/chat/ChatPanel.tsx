...
function highlightTriggerPhrases(text: string, phrases: string[]) {
  ...
  return text.split(matcher).map((part, index) => {
    ...
    return (
      <mark key={`${part}-${index}`} className="rounded bg-amber-200/80 px-1 py-0.5 text-theme-primary">
        {part}
      </mark>
    );
  });
...
function ConfidenceIndicator({ confidence }: { confidence: number }) {
  ...
  return (
    <div className="flex items-center gap-2 rounded-full border border-theme-border bg-theme-surface-solid px-2.5 py-1 text-[11px] font-medium text-theme-primary shadow-sm">
...
function MessageAccordion({ title, icon: Icon, children, defaultOpen = false }: {
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-theme-border bg-theme-surface-solid/80">
      <button
        type="button"
  ...
  return (
    <MessageAccordion title="Why this response" icon={Lightbulb} defaultOpen>
      <div className="space-y-3">
        {sourceMessage && (
          <div className="rounded-lg bg-theme-surface p-3 leading-relaxed text-theme-primary">
        ...
        <div className="grid gap-2">
          {explanationRows.map((row) => (
            <div key={row.label} className="rounded-lg border border-theme-border bg-theme-surface-solid p-3">
        ...
        {data.explanation.confidence_note && (
          <div className="rounded-lg bg-theme-surface/70 p-3 leading-relaxed text-theme-primary">
    ...
    <MessageAccordion title="Sources used" icon={FileText}>
      <div className="space-y-2">
        {data.retrieved_docs.map((doc, index) => (
          <div key={`${doc.source_file}-${doc.chunk_index}-${index}`} className="flex items-center justify-between gap-3 rounded-lg bg-theme-surface px-3 py-2">
            <span className="min-w-0 truncate font-mono text-[11px] text-theme-primary">
              {doc.source_file} · chunk {doc.chunk_index}
            </span>
            <span className="shrink-0 rounded-full bg-theme-surface-solid px-2 py-0.5 font-mono text-[10px] text-theme-muted">
  ...
  return (
    <div className="mt-3 space-y-3">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="rounded-full border border-theme-border bg-theme-surface-solid px-2.5 py-1 text-[11px] font-medium capitalize text-theme-muted">
...
export default function ChatPanel({ sessionId: initialSessionId }: { sessionId: string }) {
  ...
  return (
    <div className="flex flex-row h-full w-full bg-theme-surface">
      <div className="flex flex-col flex-1 h-full min-w-0 relative">
        <div className="flex items-center justify-between px-4 sm:px-6 py-3.5 border-b border-theme-border bg-theme-surface-solid/80 backdrop-blur-md z-10 sticky top-0 shadow-xs">
          <div className="flex items-center space-x-2 truncate min-w-0">
            <span className="w-2 h-2 rounded-full bg-[#0D9488] animate-pulse shrink-0" aria-hidden="true" />
            <span className="font-mono text-xs text-theme-primary font-medium truncate">
              {currentSessionId}
            </span>
          </div>
          <div className="flex items-center gap-2 shrink-0 ml-2">
            <button
              ...
              className="lg:hidden w-9 h-9 flex items-center justify-center rounded-lg border border-theme-border bg-theme-surface-solid text-theme-primary hover:bg-[#F1F5F9] transition-colors"
            ...
            <Button 
              ...
              className="flex-shrink-0 bg-theme-surface-solid border-theme-border text-theme-primary hover:bg-[#F1F5F9]"
            >
              New Session
            </Button>
          </div>
                  ...
                  >
                    ...
                    <div className={`
                      ...
                      ${isUser 
                        ...
                        : 'bg-theme-surface-solid text-theme-primary border border-theme-border rounded-2xl rounded-tl-sm shadow-xs'}
                    `}>
                      {isUser ? (
                    ...
                    {isUser && (
                      <div className="flex-shrink-0 mt-1">
                        <div className="w-8 h-8 rounded-full bg-theme-surface-solid border border-theme-border flex items-center justify-center shadow-xs" aria-hidden="true">
        ...
        <div className="absolute bottom-0 inset-x-0 bg-theme-surface-solid border-t border-theme-border p-3 sm:p-4 shadow-lg">
          <div className="max-w-3xl mx-auto flex items-end space-x-3 bg-theme-surface-solid">
            {wsError && (
            ...
            <textarea
              ...
              className="flex-1 rounded-xl border border-theme-border px-4 py-3 resize-none outline-none focus:border-[#0D9488] focus:ring-2 focus:ring-[#0D9488]/20 bg-theme-surface focus:bg-theme-surface-solid transition-all text-sm text-theme-primary placeholder-[#94A3B8] max-h-[120px] font-sans font-normal"
            />
            {supported && (
              <Button
                variant="secondary"
                onClick={toggleListening}
                className={`flex-shrink-0 w-12 h-[46px] p-0 flex items-center justify-center transition-colors rounded-xl border-theme-border ${
                  isListening ? 'bg-red-50 border-red-200 text-red-500 hover:bg-red-100 hover:text-red-600' : 'text-theme-muted hover:text-theme-primary bg-theme-surface-solid'
                }`}
