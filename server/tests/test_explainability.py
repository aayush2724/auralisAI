...
class TestSchema:
    ...
    def test_handoff_reason_none_or_str(self):
        result = explain(_make_state())
        assert result["handoff_reason"] is None or isinstance(
            result["handoff_reason"], str
        )
...
class TestPersonaReason:
    ...
    def test_contains_pitch_angle(self):
        result = explain(_make_state())
        assert (
            "architecture" in result["persona_reason"].lower()
            or "pitch angle" in result["persona_reason"].lower()
        )
...
class TestSentimentReason:
    def test_negative_sentiment(self):
        ...
        assert (
            "frustrated" in reason.lower()
            or "negative" in reason.lower()
            or "resistant" in reason.lower()
        )
    ...
    def test_positive_sentiment(self):
        ...
        result = explain(state)
        assert (
            "engaged" in result["sentiment_reason"].lower()
            or "receptive" in result["sentiment_reason"].lower()
            or "positive" in result["sentiment_reason"].lower()
        )
    ...
    def test_neutral_sentiment(self):
        ...
        result = explain(state)
        assert (
            "professional" in result["sentiment_reason"].lower()
            or "neutral" in result["sentiment_reason"].lower()
            or "measured" in result["sentiment_reason"].lower()
        )
...
class TestStrategyReason:
    def test_contains_strategy_name(self):
        result = explain(_make_state())
        assert (
            "roi_business_case" in result["strategy_reason"].lower()
            or "applied" in result["strategy_reason"].lower()
        )
    ...
    @pytest.mark.parametrize(
        "obj_label,persona,expected_keyword",
        [
            ("price", "CEO", "roi"),
            ("price", "Founder", "roi"),
    ...
    def test_strategy_reason_per_combination(
        self, obj_label, persona, expected_keyword
    ):
        state = _make_state(
            objection={
                "label": obj_label,
...
class TestConfidenceNote:
    ...
    def test_low_confidence_triggers_note(self):
        ...
        assert result["confidence_note"] != ""
        assert (
            "low" in result["confidence_note"].lower()
            or "uncertain" in result["confidence_note"].lower()
        )
    ...
    def test_high_frustration_triggers_note(self):
        ...
        assert result["confidence_note"] != ""
        assert (
            "escalation" in result["confidence_note"].lower()
            or "frustrat" in result["confidence_note"].lower()
        )
...
class TestHandoffReason:
    ...
    def test_handoff_reason_mentions_frustration(self):
        ...
        assert (
            "sentiment" in result["handoff_reason"].lower()
            or "frustrat" in result["handoff_reason"].lower()
        )
