...
function Accordion({ title, children, defaultOpen = false }: { title: string, children: React.ReactNode, defaultOpen?: boolean }) {
  ...
  return (
    <div className="border border-theme-border rounded-xl overflow-hidden mb-4 shadow-xs">
      <button 
        onClick={() => setIsOpen(!isOpen)}
        className="w-full flex items-center justify-between px-4 py-3 bg-theme-surface-solid hover:bg-theme-surface transition-colors text-left font-sans"
      >
      ...
      {isOpen && (
        <div className="px-4 py-3 bg-theme-surface border-t border-theme-border text-xs text-theme-primary">
          {children}
        </div>
      )}
    </div>
...
export default function DiagnosticsPanel({ data }: { data: ChatResponse | null }) {
  if (!data) {
    return (
      <div className="w-72 border-l border-theme-border bg-theme-surface-solid h-full flex flex-col items-center justify-center p-6 text-center shadow-xs">
        <div className="w-16 h-16 rounded-full bg-[#F1F5F9] border border-theme-border flex items-center justify-center mb-4">
  ...
  return (
    <motion.div 
      ...
      className="w-72 border-l border-theme-border bg-theme-surface-solid h-full flex flex-col overflow-y-auto shadow-xs"
    >
      <div className="p-5 space-y-6">
        ...
        <div>
          ...
          <div className="mt-2 w-full bg-theme-surface rounded-full h-1.5 overflow-hidden relative">
            <motion.div 
              initial={{ width: 0 }}
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {data.explanation.trigger_phrases.map((tp, idx) => (
                      <span key={idx} className="bg-theme-surface border border-theme-border text-theme-primary text-[10px] px-2 py-0.5 rounded-md font-mono">
                        "{tp}"
                      </span>
                    ))}
                  </div>
            ...
            <div className="space-y-4">
              {data.retrieved_docs.map((doc, i) => (
                <div key={i} className="space-y-1">
                  ...
                  <div className="w-full bg-theme-surface h-1 rounded-full overflow-hidden relative">
